...
// Hiyerarşik departman yapısını oluşturma fonksiyonu
function buildDepartmentHierarchy($departments) {
    $hierarchy = [];
    $departmentMap = [];
    
    // İlk geçiş: tüm departmanları map'e al
    ...
    foreach ($departmentMap as $id => $dept) {
        if ($dept['parent_id'] && isset($departmentMap[$dept['parent_id']])) {
            // Alt departman olarak ekle
            $departmentMap[$dept['parent_id']]['children'][] = &$departmentMap[$id];
            $departmentMap[$id]['level'] = $departmentMap[$dept['parent_id']]['level'] + 1;
        } else {
            // Ana departman
            $hierarchy[] = &$departmentMap[$id];
        }
    }
...
// Hiyerarşik yapıyı düz listeye çevirme fonksiyonu
function flattenHierarchy($hierarchy, $level = 0) {
    $flat = [];
    foreach ($hierarchy as $dept) {
        $dept['level'] = $level;
        $dept['indent'] = str_repeat('— ', $level);
        $children = $dept['children'];
        unset($dept['children']);
        $flat[] = $dept;
        
        if (!empty($children)) {
            $flat = array_merge($flat, flattenHierarchy($children, $level + 1));
        }
    }
    return $flat;
}
    ...
    switch ($method) {
        case 'GET':
            ...
            foreach ($columns as $column) {
                ...
                if ($column['Field'] === 'parent_id') $hasParentId = true;
            }
            ...
            if ($hasParentId) {
                $selectFields .= ", d.parent_id, p.name as parent_name";
                $joinClause = "LEFT JOIN departments p ON d.parent_id = p.id";
            } else {
                $selectFields .= ", NULL as parent_id, NULL as parent_name";
                $joinClause = "";
            }
            ...
            // Hiyerarşik sıralama için ORDER BY güncelle
            if ($hasParentId) {
                $query = "SELECT {$selectFields} FROM departments d {$joinClause} ORDER BY d.parent_id IS NULL DESC, d.parent_id ASC, d.name ASC";
            } else {
            ...
            $flatDepartments = [];
            if ($hasParentId) {
                $hierarchicalDepartments = buildDepartmentHierarchy($departments);
                $flatDepartments = flattenHierarchy($hierarchicalDepartments);
            } else {
            ...
            echo json_encode([
                ...
                'hierarchical_departments' => $hierarchicalDepartments,
                'flat_departments' => $flatDepartments,
        ...
        case 'POST':
            ...
            if (in_array('parent_id', $availableColumns) && isset($input['parent_id'])) {
                $insertFields[] = 'parent_id';
                $insertValues[] = $input['parent_id'] ?: null;
                $placeholders[] = '?';
            }
        ...
        case 'PUT':
            ...
            if (in_array('parent_id', $availableColumns)) {
                $updateParts[] = 'parent_id = ?';
                $updateValues[] = $input['parent_id'] ?: null;
            }
